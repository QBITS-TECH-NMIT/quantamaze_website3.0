(function() {
  ...
  // Elements for the shared modal that every game renders into.
  ...
  // Display copy (badge / title / description) shown in the modal header
  // for each game, keyed by the same id used in [data-qam-game].
  var GAME_META = {
    maze: {
      tag: 'ARCADE // 01',
      title: 'Q-Maze Runner',
      desc: 'Use the arrow keys, WASD, or the on-screen pad to guide the qubit from the top-left to the glowing exit.'
    },
    circuit: {
      tag: 'ARCADE // 02',
      title: 'Circuit Builder',
      desc: 'Tap gates in order to recreate the target circuit for each level.'
    },
    cipher: {
      tag: 'ARCADE // 03',
      title: 'Crack the Cipher',
      desc: 'Drag the slider to shift the letters back until the message makes sense, then submit before time runs out.'
    },
    chain: {
      tag: 'ARCADE // 04',
      title: 'Chain the Blocks',
      desc: 'Tap blocks in order to rebuild a valid chain — each block must reference the hash of the one before it.'
    }
  };

  // Opens the modal and renders the requested game (id is one of
  // "maze" / "circuit" / "cipher" / "chain"). Cleans up whatever game
  // was previously open first.
  ...
  // Hides the modal and runs the current game's cleanup function.
  function closeModal() {
  ...
  document.querySelectorAll('[data-qam-game]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      openGame(btn.getAttribute('data-qam-game'));
    });
  });
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && overlay.classList.contains('qam-open')) closeModal();
  });
  ...
  function renderMaze(container) {
    ...
    function makeMaze() {
      ...
      function neighbors(cell) {
        ...
        var x = cell.x,
          y = cell.y;
        if (y > 0 && !grid[y - 1][x].visited) list.push({
          cell: grid[y - 1][x],
          dir: 'N',
          opp: 'S'
        });
        if (y < ROWS - 1 && !grid[y + 1][x].visited) list.push({
          cell: grid[y + 1][x],
          dir: 'S',
          opp: 'N'
        });
        if (x < COLS - 1 && !grid[y][x + 1].visited) list.push({
          cell: grid[y][x + 1],
          dir: 'E',
          opp: 'W'
        });
        if (x > 0 && !grid[y][x - 1].visited) list.push({
          cell: grid[y][x - 1],
          dir: 'W',
          opp: 'E'
        });
    ...
    function draw() {
      ...
      ctx.fillRect((COLS - 1) * cellSize + cellSize * 0.25, (ROWS - 1) * cellSize + cellSize *
        0.25, cellSize * 0.5, cellSize * 0.5);
      // player
      ctx.beginPath();
      ctx.fillStyle = '#ffd27a';
      ctx.shadowColor = '#F5590A';
      ctx.shadowBlur = 12;
      ctx.arc(player.x * cellSize + cellSize / 2, player.y * cellSize + cellSize / 2, cellSize *
        0.28, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    }
  ...
  function renderCircuit(container) {
    var LEVELS = [{
        target: ['H', 'X'],
        hint: 'Hadamard puts the qubit into superposition, then Pauli-X flips it.',
        gates: ['H', 'X', 'Z', 'CX']
      },
      {
        target: ['X', 'H', 'Z'],
        hint: 'Flip, superpose, then apply a phase shift.',
        gates: ['H', 'X', 'Z', 'CX']
      },
      {
        target: ['H', 'CX', 'X', 'Z'],
        hint: 'Entangle two qubits after superposition, then flip and phase-shift.',
        gates: ['H', 'X', 'Z', 'CX']
      }
    ];
    var levelIndex = 0;
    ...
    function renderLevel() {
      ...
      current = [];
      container.innerHTML =
        '<div class="qam-row"><span class="qam-pill" id="qam-circuit-level">Level ' + (
          levelIndex + 1) + ' / ' + LEVELS.length + '</span>' +
      ...
      container.querySelector('#qam-circuit-clear').addEventListener('click', function() {
        current = [];
        drawTrack();
        refs.status.textContent = '';
        refs.status.className = 'qam-status';
      });
      container.querySelector('#qam-circuit-check').addEventListener('click', function() {
        var refs2 = el();
        var ok = current.length === lvl.target.length && current.every(function(g, i) {
          return g === lvl.target[i];
        });
        if (ok) {
          refs2.status.textContent = levelIndex === LEVELS.length - 1 ?
            'All levels cleared! You built a full circuit set.' :
  ...
  function renderCipher(container) {
    ...
    var round = 0,
      ...
      shift = 0,
      timeLeft = ROUND_TIME,
      interval, plaintext, ciphertext, answered;
    ...
    function draw() {
      container.innerHTML =
        '<div class="qam-row"><span class="qam-pill">Round ' + (round + 1) + ' / ' + PHRASES
        .length + '</span><span class="qam-pill" id="qam-cipher-time">' + ROUND_TIME +
        's</span></div>' +
      ...
      container.querySelector('#qam-cipher-slider').addEventListener('input', function(e) {
        ...
        container.querySelector('#qam-cipher-preview').textContent = shiftText(ciphertext,
          shift);
      });
    ...
    function showResults() {
      clearInterval(interval);
      container.innerHTML =
        '<p class="qam-status qam-good" style="font-size:1rem;">You cracked ' + score +
        ' out of ' + PHRASES.length + ' ciphers.</p>' +
  ...
  function renderChain(container) {
    ...
    function setup() {
      blocks = NAMES.map(function(name, i) {
        return {
          name: name,
          hash: randHash(),
          prevHash: null,
      ...
      order = blocks.slice().sort(function() {
    ...
    function draw() {
      ...
      for (var i = 0; i < blocks.length; i++) {
        ...
        if (placed[i]) {
          ...
          var b = placed[i];
          var isGood = (i === 0 && b.prevHash === '00000000') || (i > 0 && placed[i - 1] && b
            .prevHash === placed[i - 1].hash);
          slot.classList.add(isGood ? 'qam-slot-good' : 'qam-slot-bad');
          slot.innerHTML = '<span class="qam-block-name">' + b.name +
            '</span>&nbsp;<span class="qam-block-hash">hash:' + b.hash + ' · prev:' + b.prevHash +
            '</span>';
        } else {
      ...
      order.forEach(function(b) {
        ...
        el.innerHTML = '<span class="qam-block-name">' + b.name +
          '</span><span class="qam-block-hash">hash:' + b.hash + ' · prev:' + b.prevHash +
          '</span>';
        el.disabled = used;
    ...
    function checkWin() {
      ...
      var allGood = placed.every(function(b, i) {
        return (i === 0 && b.prevHash === '00000000') || (i > 0 && b.prevHash === placed[i -
          1].hash);
      });
      ...
      if (allGood) {
        ...
        status.textContent =
          'All blocks placed, but the chain is broken somewhere — try Undo or Shuffle again.';
        status.className = 'qam-status qam-bad';
      }
    }
